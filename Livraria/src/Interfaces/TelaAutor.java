...
public class TelaAutor implements Tela {
	...
	public void start() {
		...
		grid.add(new Label("Biografia: "), 0, 2);
		grid.add(txtBiografia, 1, 2);
		...
		btnCriar.setOnAction(a -> {
			if (Integer.parseInt(txtCod.getText()) == 0 || txtNome.getText() == "" || txtBiografia.getText() == "") {
		...
		vbox.getChildren().addAll(new Label("Autor"), grid, hbox, new Label("Querys"), txtQuery, hbox2);
		vbox.setSpacing(10);
	...
	public void ligacoes() {
		...
		Bindings.bindBidirectional(txtBiografia.textProperty(), cl.biografiaProperty());
	}
